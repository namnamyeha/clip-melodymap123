...
def recommend_region(choice: str, embedding: list) -> str:
    with open("place_text_features.json", "r", encoding='utf-8') as fp:
        ...
        # 가중치 계산
        similarity_sum = sum([similarity ** 3 for similarity in region_similarities.values()])
        similarity_weights = [similarity ** 3 / similarity_sum for similarity in region_similarities.values()]
...
def recommend_places_in_region(region: str, num_recommendations: int = 5) -> list:
    # 여행지 특성 파일 열기
    with open("place_text_features.json", "r", encoding='utf-8') as fp:
    ...
    # 중복되지 않도록 추천할 장소 수를 조정
    num_places_to_recommend = min(num_recommendations, len(place_similarities))

    recommended_places = np.random.choice(
        list(place_similarities.keys()),
        num_places_to_recommend,
        p=similarity_weights,
        replace=False  # Ensure no duplicate places
    )
    recommended_places = recommended_places.tolist()

    print(f"지역에 따른 '{region}': 추천여행지 {recommended_places}.")
    return recommended_places
def recommend_music(place: str, num_recommendations: int = 5) -> list:
    ...
    # 중복되지 않도록 추천할 음악 수를 조정
    num_music_to_recommend = min(num_recommendations, len(similarities))

    recommended_music = np.random.choice(
        list(similarities.keys()),
        num_music_to_recommend,
        p=similarity_weights,
        replace=False  # Ensure no duplicate music
    )
...
def process_all_recommendations(data):
    ...
    # 모든 결과를 저장하기
    with open('0000melodymap_results.json', 'w', encoding='utf-8') as file:
