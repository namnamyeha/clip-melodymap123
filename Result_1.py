...
def recommend_region(choice: str, embedding: list) -> str:
    with open("place_text_features.json", "r", encoding='utf-8') as fp:
        ...
        for feature in place_feature:
            ...
            # 텍스트 특성 벡터와 지역 특성 벡터 사이의 코사인 유사도를 계산합니다. 가중치
        ...
        recommended_region = np.random.choice(list(region_similarities.keys()), 1, p=similarity_weights)
        # recommended_region = recommended_region.tolist()
...
def recommend_music(place: str) -> list:
    ...
    recommended_music = np.random.choice(list(similarities.keys()), 5, p=similarity_weights)
...
def process_all_recommendations(data):
    ...
    for item in tqdm(data,desc="추천알고리즘 실행중"):
        ...
        for place in recommended_places:
            ...
            # 여행지별 음악 목록을 place_music_map 딕셔너리에 저장
        ...
        new_results.append({
            ...
            "place_music_map": place_music_map
        })

    # new_results 안의 모든 np.array를 변환
    for result in new_results:
        if isinstance(result['Recommended_region'], np.ndarray):
            result['Recommended_region'] = result['Recommended_region'].tolist()
        for key, value in result['Place_to_music_map'].items():
            if isinstance(value, np.ndarray):
                result['Place_to_music_map'][key] = value.tolist()

    # 모든 결과를 저장하기
    with open('00melodymap_results.json', 'w', encoding='utf-8') as file:
...
# 결과를 JSON 파일로 저장
with open('003_Recommendation_results.json', 'w', encoding='utf-8') as file:
